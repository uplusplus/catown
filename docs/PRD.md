...
> **状态说明（2026-04-15 更新）**
>
> 本章 8.1-8.3 中的 Pipeline Dashboard / 聊天流卡片描述，主要保留为**历史产品设计背景**，用于解释旧交互目标与审计/可观测诉求。
>
> **当前默认主前端已不是这些旧壳。** 当前实现以 `React + Vite + TypeScript` 的前端壳为主，代码位于 `frontend/src/`，`frontend/index.html` 仅作为 Vite shell。
>
> 其中，这个已实现壳体更准确地应视为 **React Mission Board shell**：它是从旧 Pipeline Dashboard 迁出的第一代 React 主界面实现。
>
> **当前更新的产品目标** 已进一步演化为以 `Navigation Core` 为中心的 `cockpit-first homepage`，不再以 `Mission Board` 作为目标首页的主命名。
>
> 后续前端开发、测试和交付，应优先以 cockpit-first homepage 目标为准，并区分：
>
> - `React Mission Board shell`：当前实现基线
> - `cockpit-first homepage`：当前目标 UX
>
> 相关决策见：
>
> - `docs/Frontend-UX-Freeze-Cockpit-Homepage.md`
...
### 8.1 Pipeline Dashboard（历史设计背景，不再是当前默认主前端）
...
| `frontend/index.html` | 历史上承载过 Pipeline Dashboard；当前已降级为 React/Vite 前端壳（实现期可称 React Mission Board shell） | 中 | ✅ 已迁移 |
