...
当前 SSE 推送 `content`、`tool_start`、`tool_result`，扩展为：

| 事件类型 | 字段 | 说明 |
|----------|------|------|
| `llm_call` | agent, model, turn, tokens_in, tokens_out, content_preview | LLM 调用摘要 |
| `stage_start` | stage, agent, skills_active, gate_type | 阶段开始 |
| `stage_end` | stage, duration, artifacts, status | 阶段结束 |
| `agent_message` | from, to, content, message_type | Agent 间消息 |
| `gate_blocked` | stage, artifact_summary | Gate 阻塞 |
| `skill_inject` | agent, skills_hint, skills_guide | Skill 注入 |
| `boss_instruction` | agent, content | BOSS 指令确认 |

#### 与 Pipeline Dashboard 的关系

- **Pipeline Dashboard**（§8.1）：宏观视角——阶段进度条、状态、耗时、产出物列表
- **聊天窗口卡片**（§8.3）：微观视角——Agent 的思考过程、工具调用细节、协作对话
...
补齐数据采集管道和可视化，为后续所有开发提供可观测性。详见 [ADR-010](ADR-010-monitoring-audit-visualization.md)。
...
| 0d | SSE 事件扩展 | 新增 llm_call / stage_start / stage_end / agent_message / gate_blocked / skill_inject 事件类型 | #0b |
| 0e | 聊天窗卡片前端 | 7 种卡片渲染函数 + renderMessages() 混合卡片流 + 折叠/展开/合并 | #0d |
