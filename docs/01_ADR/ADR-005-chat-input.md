...
从本 ADR 生效起，Catown 的用户交互遵循以下长期原则：

1. **禁止浏览器原生弹窗**：聊天流程中禁止使用 `alert` / `confirm` / `prompt` 一类阻断式弹窗。
2. **交互必须留在聊天窗口内**：如果系统需要用户确认、选择或审批，应在聊天流或聊天侧栏中动态渲染交互卡片/内联面板。
3. **确认动作只允许点击完成**：凡是可通过选项确认的交互，必须提供可点击按钮，不要求用户额外输入文字。
4. **文本输入通过对话获取**：如果确实需要用户提供文本内容，应通过正常聊天消息获取，而不是在弹窗或临时表单中索取。

**例外说明**：如果用户是通过聊天窗口以外的显式 UI 入口创建项目（例如侧边栏按钮、项目中心按钮、目录选择器），则不受“必须留在聊天窗口内”的约束，可以使用该 UI 自身的标准交互组件。

这条原则优先级高于局部实现便利性，后续新增交互组件和流程设计必须遵守。
