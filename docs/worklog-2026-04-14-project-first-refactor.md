...
- 前端重构计划已正式写入 TODO 和 wiki/ADR：后续阶段按 `主工作面收口 -> DetailRail 工作台化 -> state/query 收口 -> continue/resolve 刷新与错误态收口 -> activity feed 事件面定型 -> 重写 frontend tests -> 固化 dist 发布链路` 推进，而不是再回到旧 dashboard 兼容修补
- 在此基础上又进一步细化：主工作面、右侧 detail 工作台、前端 state/query、主动作反馈、测试重写、交付链路六条主线已拆成明确子任务，后续可以按 commit slice 逐条推进
