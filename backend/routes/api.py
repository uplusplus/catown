    ...
    *,
    ...
    for _attempt in range(3):
        try:
            ...
            if _attempt == 2:
                # Final failure — emit terminal event so it doesn't hang.
                logger.error(
                ...
                if parent_task_run and (parent_task_run.status or "").strip().lower() in ("running", "paused"):
                    append_task_event(
                        ...
                        payload={"error": str(followup_exc)},
                    )
                    complete_task_run(db, parent_task_run)
            else:
...
async def _continue_agent_after_tracked_run_shell_async(task_run_id: int, next_card: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Emit terminal event so it doesn't hang in "running".
...
async def send_message_stream(chatroom_id: int, message: MessageRequest, request: Request):
    ...
    async def raw_event_generator():
        ...
        try:
            ...
            # Emit interrupted event if TaskRun still running (SSE disconnect / exception).
