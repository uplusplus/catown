...
## Corpus Check
- 88 files · ~88,846 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1669 nodes · 4103 edges · 55 communities detected
- Extraction: 45% EXTRACTED · 55% INFERRED · 0% AMBIGUOUS · INFERRED: 2258 edges (avg confidence: 0.5)
...
Cohesion: 0.04
Nodes (122): AgentInfo, create_project(), _extract_memories(), get_agent(), get_config(), get_messages(), get_project(), list_agents() (+114 more)
...
Cohesion: 0.02
Nodes (63): LLMConfigModel, BaseModel, CollaborationManager, CollaborationStrategy, MultiAgentStrategy, Multiple agents collaborate on complex tasks, Select multiple agents based on task type, Manager for agent collaboration (+55 more)
...
Cohesion: 0.03
Nodes (34): AssetService, BootstrapStageExecutor, Owns the scaffold generation path until a real runtime replaces it., Minimal execution contract for stage executors under the new kernel., Owns the scaffold generation path until a real runtime replaces it., StageExecutionResult, Asset, AssetLink (+26 more)
...
Cohesion: 0.04
Nodes (24): DeleteFileTool, ListFilesTool, Tool for writing content to files, Write content to file                  Args:             file_path: Path to the, Tool for reading file contents, Tool for listing files in a directory, Initialize with optional workspace directory                  Args:, List files in directory                  Args:             directory: Directory (+16 more)
...
Cohesion: 0.04
Nodes (28): ABC, JSON Schema for a tool, Get OpenAI-compatible tool schema, Override this to define parameters schema, Registry for managing tools, List all registered tool names, Get schemas for specified tools (or all if not specified), Execute a tool by name (+20 more)
...
- **Thin community `Community 38`** (2 nodes): `config.py`, `Settings`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (2 nodes): `test_llm.py`, `test_llm()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `check.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (1 nodes): `test_backend.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (1 nodes): `check_code.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 52`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (1 nodes): `从 SOUL JSON 动态组装 system_prompt`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 54`** (1 nodes): `将 engine 事件转发到所有通用 /ws 连接`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Are the 176 inferred relationships involving `Project` (e.g. with `ProjectService` and `ProjectBootstrapCoordinator`) actually correct?**
  _`Project` has 176 INFERRED edges - model-reasoned connections that need verification._
