...
## Corpus Check
- 83 files · ~88,039 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1598 nodes · 3560 edges · 53 communities detected
- Extraction: 52% EXTRACTED · 48% INFERRED · 0% AMBIGUOUS · INFERRED: 1716 edges (avg confidence: 0.5)
...
2. `BaseTool` - 99 edges
3. `Pipeline` - 81 edges
4. `PipelineRun` - 81 edges
5. `PipelineMessage` - 81 edges
6. `PipelineStage` - 78 edges
7. `StageArtifact` - 78 edges
8. `AgentCollaborator` - 72 edges
9. `StageConfig` - 65 edges
10. `LLMClient` - 62 edges
...
Cohesion: 0.03
Nodes (139): AgentInfo, create_project(), _extract_memories(), get_agent(), get_config(), get_messages(), get_project(), list_agents() (+131 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (85): ABC, BaseTool, JSON Schema for a tool, Base class for all tools, Get OpenAI-compatible tool schema, Override this to define parameters schema, Registry for managing tools, List all registered tool names (+77 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (99): Event, LLMCall, Token 汇总统计 + 成本估算（GPT-4 pricing）, 聚合时间线：events + llm_calls + tool_calls 混合排序, 单条 LLM 调用详情（含完整 prompt 和 response）, ToolCall, Base, LLMClient (+91 more)
...
Cohesion: 0.04
Nodes (27): AssetService, BootstrapStageExecutor, Owns the scaffold generation path until a real runtime replaces it., Minimal execution contract for stage executors under the new kernel., Owns the scaffold generation path until a real runtime replaces it., StageExecutionResult, Asset, AssetLink (+19 more)
...
Cohesion: 0.07
Nodes (20): BaseModel, AgentConfigManager, load_agent_configs(), Agent 配置管理器 - 支持从文件加载配置, 从 JSON 文件加载配置                  文件格式示例：         {             "agents": {, Config, create_agent_config_from_provider(), MemoryConfig (+12 more)
...
Cohesion: 0.08
Nodes (5): MemoryItem, TestAgentConversation, TestAgentCreation, TestAgentMemory, TestAgentModel

### Community 17 - "Community 17"
Cohesion: 0.08
Nodes (4): TestAgentConfigV2, TestAgentProviderConfig, TestCreateAgentConfigFromProvider, TestModelConfig
...
- **Thin community `Community 36`** (2 nodes): `config.py`, `Settings`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (2 nodes): `test_llm.py`, `test_llm()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (1 nodes): `vite.config.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (1 nodes): `check.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (1 nodes): `test_backend.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `check_code.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 50`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (1 nodes): `从 SOUL JSON 动态组装 system_prompt`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (1 nodes): `将 engine 事件转发到所有通用 /ws 连接`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Project` connect `Community 2` to `Community 0`, `Community 1`, `Community 5`, `Community 10`, `Community 13`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `BaseTool` connect `Community 1` to `Community 0`, `Community 9`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `LLMClient` connect `Community 2` to `Community 18`, `Community 0`, `Community 10`, `Community 19`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Are the 123 inferred relationships involving `Project` (e.g. with `TestPipelineCreation` and `TestPipelineStart`) actually correct?**
  _`Project` has 123 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 79 inferred relationships involving `Pipeline` (e.g. with `TestPipelineCreation` and `TestPipelineStart`) actually correct?**
  _`Pipeline` has 79 INFERRED edges - model-reasoned connections that need verification._
- **Are the 79 inferred relationships involving `PipelineRun` (e.g. with `TestPipelineCreation` and `TestPipelineStart`) actually correct?**
  _`PipelineRun` has 79 INFERRED edges - model-reasoned connections that need verification._